...
interface ExportPopoverWithSelectionProps {
  hasData: boolean;
  name: string;
  url: string;
...
export default function ExportPopoverWithSelection({
  hasData,
  name,
  url,
  params = {},
  ...
  selectedIds = [],
  selectedCount = 0,
}: ExportPopoverWithSelectionProps) {
  ...
  const handleExportExcel = async () => {
    ...
    const fileName =
      selectedIds.length > 0
        ? `Selected_${name}_${selectedCount}_${new Date().toISOString().split('T')[0]}.xlsx`
  ...
  const handleExportJson = async () => {
    ...
    const fileName =
      selectedIds.length > 0
        ? `Selected_${name}_${selectedCount}_${new Date().toISOString().split('T')[0]}.json`
        : `${name}_${new Date().toISOString().split('T')[0]}.json`;
  ...
  const getExcelButtonText = () => {
    if (selectedCount > 0) {
      return `Excel File (${selectedCount} selected)`;
    }
    return 'Excel File';
  };

  const getJsonButtonText = () => {
    if (selectedCount > 0) {
      return `Json File (${selectedCount} selected)`;
    }
    return 'Json File';
  };
